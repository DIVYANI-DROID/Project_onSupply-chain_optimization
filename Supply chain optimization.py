...
# - hyperparameters for generative model (if separate model)
...
# A major open problem on the road to artificial intelligence is the development of incrementally learning systems that
#learn about more and more concepts over time from a stream of data. ... iCaRL learns strong classifiers and a data
...
def run(args):
    # Set default arguments & check for incompatibility if any
    ...
    if (not args.scenario=="task") and args.xdg:
        raise ValueError("'XdG' is only compatible with the Task-IL scenario.") # IL - Incremental Learning -  a
        # concept also used in Learning without forgetting
    ...
    # Store in model, how many and in what way to store exemplars
    ...
    # XdG: creating a mask for each hidden fully connected layer
    ...
    # Print some model-characteristics
    # -main model
    print("\n")
